...
const ProtocoloInfoCard = ({ protocolo }) => {
  return (
    <div className="p-8 bg-gray-700 rounded-lg shadow-md w-full max-w-4xl mx-auto mb-8">
      <div className="mb-4">
        <h2 className="text-2xl text-white font-semibold">{`Título: ${protocolo.title}`}</h2>
      </div>
      <div className="flex flex-col flex-wrap md:flex-row">
        ...
        <div className="md:w-1/2 mb-4 md:mb-0 md:pr-8">
          <div className="mb-4">
            ...
            <p className="text-white">{protocolo.keywords.join(", ")}</p>
          </div>
          <div className="mb-4">
            <label className="block text-gray-300 font-medium mb-1">Resumen</label>
            <p className="text-white">{protocolo.abstract}</p>
          </div>
          <div className="mb-4">
            <label className="block text-gray-300 font-medium mb-1">Fecha de creación</label>
            <p className="text-white">{protocolo.createdAt.substring(0,10)}</p>
          </div>
        </div>
        ...
        <div className="md:w-1/2 space-y-6">
          <div>
            ...
            <div className="flex items-center">
              <span className={`w-3 h-3 rounded-full mr-2 ${getStatusCircleColor(protocolo.state)}`}></span>
              <p className={getStatusColor(protocolo.state)}>{protocolo.state}</p>
            </div>
          </div>
        </div>

        <div className=" w-full mt-5">
          <iframe src={protocolo.fileUrl} width="100%" height="600"></iframe>
        </div>
      </div>
    </div>
  );
